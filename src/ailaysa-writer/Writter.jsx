...
const Writter = (props) => {
    ...
    const getArticle = async (order_list, blog_id, lang) => {
        ...
        await fetchEventSource(`${Config.AI_GEN_URL}/openai/stream_article/?outline_section_list=${order_list}&blog_creation=${blog_id}`, {
            ...
            onmessage(event) {
                ...
                // console.log(getCaretTopPoint(document.querySelector('.note-editable')))
    ...
    const getHtmlDataForDocument = (id) => {
        isInitialHtmlDataLoaded.current = true
        // console.log('gethtml data for document')
        // summerNoteEditorRef.current.summernote('saveRange')
        Config.axios({
        ...
        Config.axios({
            ...
            success: (response) => {
                if (response.data?.length !== 0) {
                    ...
                    // console.log(response.data?.find(each => each.assign_to_details?.id == Config.userState?.id));
    ...
    const saveHtmlDataForPdf = async (target) => {
        ...
        Config.axios({
            ...
            success: (response) => {
                ...
                // console.log("from pdf "+ isTaskAssignedToMeRef.current);
    ...
    const translateDocument = async () => {
        ...
        if (!isCoAuthor && (docParam || createdDocumentId.current !== null)) {
            // console.log(await getHtmlToDocxFile())
            ...
            success: async (response) => {
                ...
                if (from === 'image-upload') {
                    ...
                    if (data.status === 200) {
                        ...
                        // console.log(`${Config.BASE_URL}${response.image}`)
    ...
    const saveTranscriptionData = async (target) => {
        ...
        Config.axios({
            ...
            success: (response) => {
                ...
                // console.log("from transcription "+ isTaskAssignedToMeRef.current);
                if (isTaskAssignedToMeRef.current && !isTaskInProgressRef.current) {
    ...
    function checkSelection() {
        // console.log(window.getSelection()?.toString() !== 0)
    ...
    const getAiCustomizationResult = (id, sel_customization, sel_category, src, tar, mtengin) => {
        ...
        // console.log(selectedCustomizationCategory)
        if (window.getSelection().type === 'Range' || sel_customization === 'Continue Writing') {
            ...
            // console.log(toggleWritingStateRef.current)
            if (sel_customization == 'Continue Writing' && toggleWritingStateRef.current === 1) {
                // console.log(selectedToneRef.current)
                ...
                success: (response) => {
                    ...
                    if (sel_customization != 'Continue Writing') {
                        ...
                        if (id != 23) {//non translation 
                            ...
                            // console.log(defaultSettings)
                            // console.log((defaultSettings?.newline !== null && defaultSettings?.newline !== undefined) && (defaultSettings?.append !== null && defaultSettings?.append !== undefined))
                            // console.log(defaultSettings)
                            if (
                                (defaultSettings?.newline !== null && defaultSettings?.newline !== undefined) && 
                                (defaultSettings?.append !== null && defaultSettings?.append !== undefined) &&
                        ...
                        // console.log(countinueWrittingResult)
    ...
    const replaceWithNewText = (e, selectedCustomization) => {
        ...
        // console.log(selectedCustomizationCategoryRef.current)
        let p_start_tag = '<p>'
    ...
    const selectTheInsertedNodeText = (range, node) => {
        ...
        try{
            ...
            console.log("insernote text errornote: " +e)
        }
    } 
            ...
            if (selectedCustomizationCategoryRef.current == 'Convert') {
                if (selectedCustomization == 'Paragraph to bullet points') {    // convert category - paragraph to bullet and vice-versa
                    ...
                    // add the new data in same or next line (don't delete the selected text)
                        ...
                        console.log('insert without replacing') 
                        if(new_line) {
                            console.log('new line')
    ...
    const handleBtnClick = (param) => {
        ...
        if (!isCollapsed){
            ...
            // console.log(rng.getClientRects())
    ...
    const uploadImagesToServer = async (files) => {
        ...
        // console.log(URL_SEARCH_PARAMS.get("document-id"))
