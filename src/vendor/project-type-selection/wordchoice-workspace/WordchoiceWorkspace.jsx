...
const WordchoiceWorkspace = (props) => {
    ...
    //                 ...(isFromEdit && { pos: termCopy?.pos?.trim() })
    //             }
    //         }
    //         return obj
    //     })
    ...
    const handleTermsDownload = async() => {
        try{
            ...
            dispatch(addDownloadingFiles({ id: selectedTaskItem?.value, file_name: languagePairObject?.project_name, ext: '.xlxs', status: 1 }))

            let url = `${Config.BASE_URL}/glex/terms_simple_download/?task=${selectedTaskItem?.value}`
            const response = await Config.downloadFileFromApi(url);
            
            // update the list once download completed
            dispatch(updateDownloadingFile({ id: selectedTaskItem?.value, status: 2 }))

            Config.downloadFileInBrowser(response)

            setTimeout(() => {
                // remove the downloaded file from list
                dispatch(deleteDownloadingFile({ id: selectedTaskItem?.value }))
            }, 8000);
