...
const CreateGlossaries = (props) => {
		...
		Config.axios({
			...
			success: (response) => {
				...
				setTimeout(() => {
					...
					if (
						contentprojectNameRef.current !== undefined &&
						contentprojectNameRef.current !== null
					) {
	...
	const handleUpdate = (e, submission) => {
		...
		if (primaryGlossarySourceName !== "") {
			formData.append(
				...
				primaryGlossarySourceName !== null ? primaryGlossarySourceName : ""
			);
		}
		if (glossaryCopyrightOwner !== "") {
			formData.append(
				"source_Copyright_owner",
				glossaryCopyrightOwner !== null ? glossaryCopyrightOwner : ""
			);
		}
		if (detailsOfPrimaryGlossarySourceName !== "") {
			formData.append(
				"details_of_PGS",
				detailsOfPrimaryGlossarySourceName !== null
	...
	const handleBlockedNavigation = ({ nextLocation }) => {
		if (
			glossaryProjectCreationResponse === null &&
			projectName &&
			sourceLanguage &&
			targetLanguage &&
			!nextLocation.pathname.includes("/assets")
		) {
			return true;
		}
		return false;
